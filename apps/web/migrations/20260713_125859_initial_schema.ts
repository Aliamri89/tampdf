...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`static_pages_key_idx\` ON \`static_pages\` (\`key\`);`)
  ...
  await db.run(sql`CREATE TABLE \`static_pages_locales\` (
  	\`title\` text NOT NULL,
  	\`content\` text NOT NULL,
  	\`seo_seo_title\` text,
  	\`seo_seo_description\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`static_pages\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`static_pages_locales_locale_parent_id_unique\` ON \`static_pages_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`tool_usage_events\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`tool\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`tool_usage_events_updated_at_idx\` ON \`tool_usage_events\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`tool_usage_events_created_at_idx\` ON \`tool_usage_events\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`page_visit_events\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`page_visit_events_updated_at_idx\` ON \`page_visit_events\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`page_visit_events_created_at_idx\` ON \`page_visit_events\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	\`media_id\` integer,
  	\`tool_usage_events_id\` integer,
  	\`page_visit_events_id\` integer,
  	\`users_id\` integer,
  	...
  	FOREIGN KEY (\`tool_usage_events_id\`) REFERENCES \`tool_usage_events\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`page_visit_events_id\`) REFERENCES \`page_visit_events\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_tool_usage_events_id_idx\` ON \`payload_locked_documents_rels\` (\`tool_usage_events_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_page_visit_events_id_idx\` ON \`payload_locked_documents_rels\` (\`page_visit_events_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`settings\` (
  	...
  	\`analytics_clarity_project_id\` text,
  	\`updated_at\` text,
  	\`created_at\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`static_pages_locales\`;`)
  await db.run(sql`DROP TABLE \`media\`;`)
  await db.run(sql`DROP TABLE \`tool_usage_events\`;`)
  await db.run(sql`DROP TABLE \`page_visit_events\`;`)
